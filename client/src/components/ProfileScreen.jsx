...
export function ProfileScreen({ volunteerId, onLogout, reports, onResolve }) {
  ...
  const conflicts = reports ? reports.filter(r => r.syncStatus === 'conflict') : [];
  ...
  return (
    <div className="screen-content hide-scrollbar" style={{ overflowY: 'auto' }}>
      ...
      {conflicts.length > 0 && (
        <>
          <h2 className="heading-md" style={{ fontSize: 20, marginBottom: 16, color: 'var(--primary-red)' }}>Pending Conflicts</h2>
          <div className="card flex-col gap-16" style={{ marginBottom: 24, border: '1px solid var(--primary-red)' }}>
             <ReportList reports={conflicts} onResolve={onResolve} />
          </div>
        </>
      )}
